...
from vnpy.app.cta_strategy.tqz_constant import TQZPositionKeyType
from vnpy.app.cta_strategy.tqz_logging import TQZLogging
...
class CtaEngine(BaseEngine):
    ...
    ) + f'/.vntrader/cta_strategy_setting.json'

    data_filename = TQZFilePathOperator.current_file_grandfather_path(
        file=TQZFilePathOperator.grandfather_path(source_path=__file__)
    ) + f'/.vntrader/cta_strategy_data.json'
    ...
    def __init__(self, main_engine: MainEngine, event_engine: EventEngine):
        ...
        strategy_data = TQZPositionJsonOperator.tqz_get_single_jsonfile_format_data(jsonfile=self.data_filename)
        self.last_strategy_data_format: dict = TQZPositionJsonOperator.tqz_get_sum_position_format_data_with_jsonfileContentList(strategy_data)
    ...
    @staticmethod
    def query_bar_from_rq(symbol: str, exchange: Exchange, interval: Interval, start: datetime, end: datetime):
        """
        ...
        for current_vt_symbol in current_strategy_data_format.keys():
            ...
            if current_vt_symbol in last_strategy_data_format.keys():  # last_strategy_data have current_vt_symbol
                ...
                if current_strategy_data_format[current_vt_symbol][TQZPositionKeyType.BUY_POSITION_KEY.value] is not last_strategy_data_format[current_vt_symbol][TQZPositionKeyType.BUY_POSITION_KEY.value]:
                    message = f'[strategy position change][vt_symbol: {str(current_vt_symbol)}, pre buy position: {str(last_strategy_data_format[current_vt_symbol][TQZPositionKeyType.BUY_POSITION_KEY.value])}, current buy position: {str(current_strategy_data_format[current_vt_symbol][TQZPositionKeyType.BUY_POSITION_KEY.value])}]'
                ...
                if current_strategy_data_format[current_vt_symbol][TQZPositionKeyType.SELL_POSITION_KEY.value] is not last_strategy_data_format[current_vt_symbol][TQZPositionKeyType.SELL_POSITION_KEY.value]:
                    message = f'[strategy position change][vt_symbol: {str(current_vt_symbol)}, pre sell position: {str(last_strategy_data_format[current_vt_symbol][TQZPositionKeyType.SELL_POSITION_KEY.value])}, current sell position: {str(current_strategy_data_format[current_vt_symbol][TQZPositionKeyType.SELL_POSITION_KEY.value])}]'
                    message_list.append(message)
                ...
                if current_strategy_data_format[current_vt_symbol][TQZPositionKeyType.BUY_POSITION_KEY.value] is not 0:
                    message = f'[strategy position change][vt_symbol: {str(current_vt_symbol)}, pre buy position: {str(0)}, current buy position: {str(current_strategy_data_format[current_vt_symbol][TQZPositionKeyType.BUY_POSITION_KEY.value])}]'
                    message_list.append(message)
                if current_strategy_data_format[current_vt_symbol][TQZPositionKeyType.SELL_POSITION_KEY.value] is not 0:
                    message = f'[strategy position change][vt_symbol: {str(current_vt_symbol)}, pre sell position: {str(0)}, current sell position: {str(current_strategy_data_format[current_vt_symbol][TQZPositionKeyType.SELL_POSITION_KEY.value])}]'
                    message_list.append(message)
            ...
            if vt_symbol_direction in pre_positions.keys():  # pre_positions data have this vt_symbol_direction
                ...
                if position_data.direction in [Direction.LONG]:  # judge position_data direction
                    if position_data.volume is not pre_positions[vt_symbol_direction].volume:
                        message = f'[real position change][vt_symbol: {str(position_data.vt_symbol)}, pre buy position: {str(pre_positions[vt_symbol_direction].volume)}, current buy position: {str(position_data.volume)}]'
                elif position_data.direction in [Direction.SHORT]:
                    if position_data.volume is not pre_positions[vt_symbol_direction].volume:
                        message = f'[real position change][vt_symbol: {str(position_data.vt_symbol)}, pre sell position: {str(pre_positions[vt_symbol_direction].volume)}, current sell position: {str(position_data.volume)}]'

            else:  # pre_positions data have not vt_symbol_direction

                if position_data.direction in [Direction.LONG]:  # judge position_data direction
                    if position_data.volume is not 0:
                        message = f'[real position change][vt_symbol: {str(position_data.vt_symbol)}, pre buy position: {str(0)}, current buy position: {str(position_data.volume)}]'
                elif position_data.direction in [Direction.SHORT]:
                    if position_data.volume is not 0:
                        message = f'[real position change][vt_symbol: {str(position_data.vt_symbol)}, pre sell position: {str(0)}, current sell position: {str(position_data.volume)}]'
    ...
    def load_bar(self, vt_symbol: str, days: int, interval: Interval, callback: Callable[[BarData], None], use_database: bool):
        """"""
        ...
        if not bars:
            bars = database_manager.load_bar_data(symbol=symbol, exchange=exchange, interval=interval, start=start,end=end)

        for bar in bars:
            callback(bar)

    @staticmethod
    def load_tick(vt_symbol: str, days: int, callback: Callable[[TickData], None]):
        """"""
        symbol, exchange = extract_vt_symbol(vt_symbol)
        end = datetime.now()
        start = end - timedelta(days)

        ticks = database_manager.load_tick_data(symbol=symbol, exchange=exchange, start=start, end=end)
    ...
    def _init_strategy(self, strategy_name: str):
        ...
        print("在策略初始化内部，现在的data是", data)

        # Call on_init function of strategy
        print("在策略初始化内部，现在call策略的初始化部件")
    ...
    def start_strategy(self, strategy_name: str):
        ...
        strategy.pos = self.strategy_data[strategy.strategy_name]['pos']  # re set strategy pos before start strategy
    ...
    def load_strategy_data(self):
        ...
        self.strategy_data = TQZPositionJsonOperator.tqz_load_jsonfile(jsonfile=self.data_filename)
    ...
    def sync_strategy_data(self, strategy: CtaTemplate):
        ...
        TQZPositionJsonOperator.tqz_write_jsonfile(content=self.strategy_data, target_jsonfile=self.data_filename)
    ...
    def load_strategy_setting(self):
        ...
        Load setting file.
        """
        self.strategy_setting = TQZPositionJsonOperator.tqz_load_jsonfile(jsonfile=self.setting_filename)
    ...
    def update_strategy_setting(self, strategy_name: str, setting: dict):
        ...
        TQZPositionJsonOperator.tqz_write_jsonfile(content=self.strategy_setting, target_jsonfile=self.setting_filename)
    ...
    @staticmethod
    def write_bug_log(vt_symbol: str, message: str):
        """
        Write strategy bug log.
        """

        TQZLogging.write_bug_log(vt_symbol=vt_symbol, message=message)